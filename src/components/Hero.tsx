  ...
  return (
    <section className="relative pb-10 min-h-screen flex items-center pt-20 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-10 relative z-10 w-full">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            <div className="glass mt-7 p-6 md:p-12 rounded-[40px] max-w-2xl w-full text-center lg:text-left">
              <div className="inline-block px-4 py-1.5 bg-brand-gold text-white text-[10px] md:text-xs font-bold tracking-widest uppercase rounded-full mb-6">
                <TypingText text="Professional Construction Services" delay={0.8} />
              </div>
              
              <h1 className="text-[clamp(2.25rem,3.5vw,3.75rem)] leading-[1.05] font-bold text-black tracking-tighter mb-6 break-words">
                <BlurReveal text="QUALITY" delay={0.2} /><br />
                <BlurReveal text="CONSTRUCTION" delay={0.4} /><br />
              ...
              <p className="text-base md:text-lg text-black/70 mb-8 max-w-md mx-auto lg:mx-0">
                Reliable building, plumbing & finishing services across Johannesburg. We build trust, not just structures.
              </p>
              ...
              <div className="flex flex-col sm:flex-row flex-wrap justify-center lg:justify-start gap-4">
                ...
                <div className="flex items-center justify-center gap-3 px-6 py-4 glass rounded-full">
                  ...
                  <span className="text-sm font-medium">
                    <Counter value={500} suffix="+" duration={2.5} /> Happy Clients
                  </span>
                </div>
              </div>
            </div>
          ...
          >
            <div className="relative">
              ...
              >
                <div className="text-[clamp(1.5rem,4vw,2.5rem)] font-bold text-brand-gold mb-1 leading-none">
                  <Counter value={15} suffix="+" />
                </div>
                <div className="text-[10px] md:text-xs font-bold uppercase tracking-widest text-black/60">Years of Excellence</div>
              </motion.div>
            </div>
          </motion.div>
        </div>
        ...
        >
          {['Skilled Team', 'Fast Response', 'Quality Work', 'Trusted Results'].map((item) => (
            <div key={item} className="flex items-center gap-2 text-white/80 font-medium tracking-wide">
              <div className="w-5 h-5 rounded-full bg-brand-gold flex items-center justify-center text-white text-[10px]">✓</div>
              {item}
            </div>
          ))}
        </motion.div>
